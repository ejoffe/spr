...
func BranchNameFromCommit(gitcmd GitInterface, cfg *config.Config, commit Commit) string {
	remoteBranchName := cfg.Repo.GitHubBranch
	return GetLocalBranchName(gitcmd) + "/spr/" + remoteBranchName + "/" + commit.CommitID
}
